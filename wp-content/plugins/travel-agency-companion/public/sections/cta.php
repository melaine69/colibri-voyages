...
<section id="cta_section" class="cta"<?php echo $bg_img; ?>>
	<div class="container">
		<div class="text wow fadeInUp" data-wow-duration="1s" data-wow-delay="0.1s">
			<?php
			if ( $title ) {
				echo '<h2 class="title">' . esc_html( travel_agency_companion_get_cta_title() ) . '</h2>';
			}
			if ( $content ) {
				echo '<div class="content">' . wp_kses_post( travel_agency_companion_get_cta_content() ) . '</div>';
			}
			if ( $readmore && $more_link ) {
				echo '<a href="' . esc_url( $more_link ) . '" class="btn-more">' . esc_html( travel_agency_companion_get_ctabtn_label() ) . '</a>';
			}
			?>
		</div>
	</div>
